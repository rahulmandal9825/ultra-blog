...
const PostCard = ({post}) => {
    ...
    return (
        <div className="flex flex-col gap-3 p-4">
            <div className="">
            <Image src={post.image} alt="" className=" rounded-lg" height={400} width={300}/>
            </div>
            ...
            <div className=" font-bold flex justify-between h-[150px] flex-col">
                <div>
                    <h1>{post.title}</h1>
                <h1 className="text-sm font-normal text-gray-500">{post.desc.slice(0,100)}...</h1>
                </div>
                
                <Link className="text-sm" href={`/blog/${post._id}`}>
                    Read more...
                </Link>
            </div>
        </div>
